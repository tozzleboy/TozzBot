    ...
    runCommand(args, msgObject, client) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let UserEmbed = new Discord.MessageEmbed()
                ...
                .setDescription(`You recieved a warning in ${msgObject.member.guild.name}!\nReason: ${args.slice(1).join(" ")}`)

            let ModEmbed = new Discord.MessageEmbed()
                .setColor("PURPLE")
                .setDescription(`${wUser} has recieved a warning!\nReason: ${args.slice(1).join(" ")}`)
                .addField(`User Current amoung of warnings: ${warns[wUser.id].warns}`)
            msgObject.channel.send(ModEmbed)
            wUser.send(UserEmbed)

            if (warns[wUser.id].warns == 3) {
                let Extension = new Discord.MessageEmbed()
                    .setColor("RED")
                    .setDescription(`**${wUser} has recieved 3 warnings, this user is eligible for punishment.**`)
                msgObject.channel.send(Extension)
            }
